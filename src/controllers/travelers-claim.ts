...
export async function createPayment(params: CreatePaymentParams) {
  ...
  try {
    // 1. Search Claim
    logger.info('Searching claim');
    const searchClaimBody = JSON.stringify({
      Conditions: {},
      PageNo: 1,
      PageSize: 10,
      FuzzyConditions: { ExtClaimNo: params.newEcoFnolId },
      Module: 'ClaimCase',
    ...
    if (!searchClaimResponse.ok) {
      ...
      throw new Error('Search Claim failed');
    }
    ...
    const mediumLoss = parseFloat(
      getDamageSeverityThresholdData.find(
        (d) => d.Description === 'MediumLoss',
      )!.Code as string,
    );
    ...
    if (params.estimatedLoss < mediumLoss) {
      damageSeverity = damageSeverityList.find(
        (d) => d.Description === 'Small',
      )!.Code as string;
    } else if (params.estimatedLoss < highLoss) {
      damageSeverity = damageSeverityList.find(
        (d) => d.Description === 'Medium',
      )!.Code as string;
    } else {
    ...
    subclaim.InsuredId =
      claimCase.ClaimEntity.PolicyEntity.InsuredList[0]['@pk'];
    ...
    if (loadClaimSettlementData.Status !== 'OK') {
      logger.error(
        `Failed to load claim settlement: ${loadClaimSettlementData}`,
      );
      throw new Error('Failed to load claim settlement');
    }
